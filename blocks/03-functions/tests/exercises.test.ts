...
import { sum, clamp, mapArray, greet } from "../src/exercises";

describe("Block 03 — Functions", () => {
  ...
  it("clamp constrains a value to the range", () => {
    expect(clamp(5, 0, 10)).toBe(5); // already inside
    expect(clamp(-3, 0, 10)).toBe(0); // below min
    expect(clamp(99, 0, 10)).toBe(10); // above max
    expect(clamp(0, 0, 10)).toBe(0); // on the boundary
  });
